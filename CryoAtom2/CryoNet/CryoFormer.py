...
import math
import einops
from CryoAtom2.CryoNet.common_modules import ThreeD_Rope
from CryoAtom2.CryoNet.sequence_attention import SequenceAttention
class NodeAttention(nn.Module):
    def __init__(
        ...
        self.bia = nn.Sequential(nn.LayerNorm(in_features//2),
                                  nn.Linear(in_features//2,attention_heads,bias=False))
    ...
    def forward(self,x_1,x_2,pos_emb,edge_index):
        ...
        query,key = ThreeD_Rope(query,key,pos_emb,edge_index)
...
class OutProductMean(nn.Module):
    def __init__(self,in_features:int,c:int = 32):
        ...
        self.c = c
        self.line2 = nn.Linear(c**2,in_features//2,bias=False)
...
class EdgeAttention(nn.Module):
    def __init__(self, in_features: int, num_neighbours: int = 30, attention_heads: int = 4,
...
class TraingleAttention(nn.Module):
    def __init__(self,in_features:int,num_neighbours:int=30,attention_heads:int=4,attention_features:int=32):
        super().__init__()
        self.ifz = in_features
        self.kz = num_neighbours
        self.ahz = attention_heads
...
class CryoFormer(nn.Module):
    def __init__(
            ...
            do_seq_attn:bool = True,
    ):
        super().__init__()
        self.do_seq_attn = do_seq_attn
        self.nodeattention = NodeAttention(in_features,num_neighbours,attention_heads,attention_features)
        if self.do_seq_attn:
            self.sequence_attention = SequenceAttention(sequence_features=1280+641,
        ...
        self.edgeattention = EdgeAttention(in_features//2,num_neighbours,attention_heads,attention_features//3*2)
        self.transition2 = Transition(in_features//2,nn.LayerNorm)
    def forward(
            self,
            x_1,
            x_2,
        ...
        if self.do_seq_attn:
            y_1,attention_scores = self.sequence_attention(y_1,packed_sequence_emb,packed_sequence_mask,prot_mask,batch,attention_batch_size)
        y_1 = self.transition1(y_1)
        y_2 = self.norm(math.sqrt(2)*x_2 + self.outer(y_1,edge_index))
        y_2 = self.edgeattention(y_2,edge_index)
