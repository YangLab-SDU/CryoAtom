...
from CryoAtom2.utils.affine_utils import affine_mul_vecs, invert_affine
...
class InvariantPointAttention(nn.Module):
    def __init__(self,in_features:int,attention_heads:int=12,c:int=48,query_points:int=4,point_values:int=8):
        ...
        self.back = nn.Linear(self.ahz*self.afz+self.ahz*(self.ifz//2)+self.ahz*self.vpz*4,self.ifz,bias=False)
    ...
    def forward(self,x1,x2,affines,pos_emb,edge_index):
        ...
        query1,key1 = ThreeD_Rope(query1,key1,pos_emb,edge_index)
...
class LinearWithSeq(nn.Module):
    def __init__(self,in_features:int,hidden_features:int,out_features:int,activate_class:nn.Module=nn.ReLU):
        super().__init__()
        self.activate_function = activate_class()
        self.ipa_linear1 = nn.Linear(in_features,hidden_features,bias=False)
        self.ipa_linear2 = nn.Linear(in_features,hidden_features)
        self.lin1 = nn.Sequential(
            self.activate_function,
            nn.Linear(hidden_features, hidden_features),
...
class LinearWithEdge(nn.Module):
    def __init__(self,in_features:int,out_features:int,activate_class:nn.Module=nn.ReLU):
        super().__init__()
        self.activate_function = activate_class()
        self.ipa_linear1 = nn.Linear(in_features*2,in_features*2)
        self.lin1 = nn.Sequential(
            self.activate_function,
    ...
    def forward(self,x1,x2):
        x = self.ipa_linear1(torch.cat([x1,x2],dim=-1))
        x = x + self.lin1(x)
        x = self.lin3(x)
        return x