...
from CryoAtom2.utils.affine_utils import affine_composition,quaternion_to_matrix,get_affine
...
class BackboneFrameNet(nn.Module):
    def __init__(self, in_features: int) -> None:
        ...
        self.bias = nn.Parameter(torch.tensor(1.5,dtype=torch.float))
        self.eps = 1e-6
    def forward(self, x, affine):
        y = self.backbone_fc(x)
        y = torch.cat([torch.sqrt(torch.square(self.bias)+self.eps)*torch.ones(size=y.shape[:-1]+(1,),dtype=torch.float,device=y.device),y],dim=-1)
        quaternions = y[...,:4]
        translations = y[...,4:]
