...
from CryoAtom2.utils.torch_utlis import get_batch_slices, padded_sequence_softmax
...
class SequenceAttention(nn.Module):
    ...
    def _intern_forward(
        ...
        batched_mask = packed_sequence_mask[batch]  # (n, seq_len, 2)
        # Since sequence emb was padded, do not consider the padded parts for attention

        sequence_attention_weights = (sequence_attention_scores-1e3*batched_mask[torch.arange(len(prot_mask),device=prot_mask.device),...,prot_mask.to(torch.long)][...,None]).softmax(dim=1)
