...
import numpy as np
import torch
from Bio.PDB import MMCIFParser, PDBParser, is_nucleic

import CryoAtom2.utils.residue_constants as _rc
from CryoAtom2.utils.affine_utils import (
    affine_composition,
    ...
    invert_affine,
    vecs_to_local_affine
)
from CryoAtom2.utils.sequence_transformer import FASTASequence,run_transformer_on_fasta
import esm
import fm
...
class PNAComplex:
    ...
    # Neighbor ID of the residue in the sequence
    edge_links: np.ndarray # (num_res,)
    keys = PNACOMPLEX_KEYS


def get_PNAComplex_from_file_path(file_path: str, chain_id: str = None) -> PNAComplex:
    ...
    for chain in model:
        ...
        for res in chain:
            ...
            if not restype_idx<_rc.canonical_num_residues:
                continue
            if flag_temp == 0:
                edge_links1.append(link_index)
            else:
                edge_links1.append(link_index - 1)
            flag_temp = 1
            link_index = link_index + 1
            edge_links0.append(link_index)
            ...
            for atom in res:
                if atom.name not in _rc.restype3_to_atoms_index[res.resname]:
                    continue
                if maskc[_rc.restype3_to_atoms_index[res.resname][atom.name]] == 1.0:
                    continue
            ...
            if restype_idx<_rc.num_prot:
                restype_sign = '<'
            elif restype_idx<_rc.num_prot+4:
                restype_sign = '='
            elif restype_idx<_rc.num_prot+8:
                restype_sign = '>'
            chain_aatype.append(restype_idx)
            if is_nucleic(res):
                posc_tensor = torch.from_numpy(posc)
                nucleic_affine0 = affine_from_3_points(posc_tensor[2],posc_tensor[1],posc_tensor[0])
                # Mirror x and z axis
                nucleic_affine0[:, 0] = nucleic_affine0[:, 0] * (-1)
                nucleic_affine0[:, 2] = nucleic_affine0[:, 2] * (-1)
                if vecs_to_local_affine(nucleic_affine0,posc_tensor[3])[-1]<-0.5:
                    change_backbone_atom_coord = np.array(posc[0])
                    posc[0] = np.array(posc[2])
                    posc[2] = change_backbone_atom_coord
                    change_backbone_atom_coord = np.array(pos[36])
                    pos[36] = np.array(pos[38])
                    pos[38] = change_backbone_atom_coord
        ...
        # if len(chain_seq[1:]) < len(chain_aatype):
    ...
    return PNAComplex(
        ...
        edge_links = np.array([edge_links0,edge_links1]).transpose(1,0),
        **frames,
        **torsion_angles,
    )


def get_PNAComplex_empty_except(**kwargs) -> PNAComplex:
    protein_dict = dict.fromkeys(PNACOMPLEX_KEYS)
    for (k, v) in kwargs.items():
        protein_dict[k] = v
    return PNAComplex(**protein_dict)
...
def atomf_to_frames(
    ...
    for restype, restype_letter in enumerate(_rc.index_to_restype_1):
        ...
        if _rc.restype3_is_prot(resname):
            ...
            # 7: nu0
            restype_rigidgroup_base_atom_names[restype, 7, :] = _rc.nuc_torsion_frames[resname][-1][1:]
            # 8: chi1
...
def atomf_to_torsion_angles(
    ...
    torsion_angles_mask[prot_mask, 2] = (
        ...
        * all_atom_mask[prot_mask, 3]  # this N, CA, C
    )  # this O
...
def torsion_angles_to_frames(
    ...
    all_frames_to_backb[~prot_mask, 8] = affine_composition(
        all_frames_to_backb[~prot_mask, 7], all_frames[~prot_mask, 8]
    )  # chi, from nu1
...
def add_lm_embeddings_to_protein(
    input_protein: PNAComplex, lm_embeddings: np.ndarray
) -> PNAComplex:
...
def make_up_fr_from_prot(prot: PNAComplex):
    final_results = {}
...
def slice_protein(protein: PNAComplex, slice_array: np.ndarray) -> PNAComplex:
    num_res = len(protein.aatype)
    for key in PNACOMPLEX_KEYS:
...
def Process_seq_embedding(lm_embeddings,prot_mask):

    sample_prot_mask = prot_mask
    sample_lm_embeddings = lm_embeddings
    seq_embedding = np.zeros((len(sample_lm_embeddings), 1280 + 641))
    seq_mask = np.zeros((len(seq_embedding), 2))
    seq_embedding[sample_prot_mask, :1280] = sample_lm_embeddings[sample_prot_mask]
    seq_embedding[~sample_prot_mask, 1280:] = sample_lm_embeddings[~sample_prot_mask, :641]
    seq_mask[sample_prot_mask, 0] = 1
    seq_mask[~sample_prot_mask, 1] = 1
    return seq_embedding,seq_mask
def get_lm_emb_to_PNAcomplex(prot_complex,max_chain_length:int=1000,device='cpu'):
    lang_model_p, alphabet_p = esm.pretrained.esm2_t33_650M_UR50D()
    batch_convert_p = alphabet_p.get_batch_converter()
    lang_model_r, alphabet_r = fm.pretrained.rna_fm_t12()
    batch_converter_r = alphabet_r.get_batch_converter()
    lang_model = {'protein': lang_model_p.eval().to(device), 'na': lang_model_r.eval().to(device)}
    batch_converter = {'protein': batch_convert_p, 'na': batch_converter_r}
    repr_layers = {'protein': [33], 'na': [12]}
    restype_sign = {'<': 'protein', '=': 'na', '>': 'na'}

    sequences_raw = prot_complex.unified_seq
    sequences_raw = sequences_raw.split("|||")
    sequences = [FASTASequence(seq[1:], restype_sign[seq[0]], "A") for seq in sequences_raw]
    ...
    lm_embeddings = []
    seq_masks = []
    for ii,ss in enumerate(seq_names):
        aatype_sign = sequences_raw[ii][0]
        temp_lm_embedding = np.zeros((len(sequences_raw[ii][1:]), 1280 + 641))
        lm_embedding = result[ss]["representations"].cpu().numpy()
        seq_mask = np.zeros((len(temp_lm_embedding), 2))
        if aatype_sign == '<':
            temp_lm_embedding[:,:1280] = lm_embedding
            seq_mask[:,0] = 1
        elif aatype_sign == '>':
            temp_lm_embedding[:,1280:] =lm_embedding[:,:641]
            seq_mask[:,1] = 1
        elif aatype_sign == '=':
            temp_lm_embedding[:,1280:] =lm_embedding[:,:641]
            seq_mask[:,1] = 1
            temp_lm_embedding[:,-1] = 1
        lm_embeddings.append(temp_lm_embedding)
        seq_masks.append(seq_mask)
    lm_embeddings = np.concatenate(
        lm_embeddings,
        axis=0,
    )
    seq_masks = np.concatenate(seq_masks,axis=0)
    assert len(lm_embeddings) == prot_complex.unified_seq_len
    # is_dna = np.logical_and(prot_complex.aatype>19,prot_complex.aatype<24)
    # lm_embeddings[is_dna,640] = 1
    # lm_embeddings,seq_emb_masks = Process_seq_embedding(lm_embeddings,prot_complex.prot_mask)
    return add_lm_embeddings_to_protein(prot_complex,lm_embeddings),seq_masks
