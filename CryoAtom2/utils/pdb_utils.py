...
from CryoAtom2.utils.residue_constants import restype_3to1,rigid_group_atom_positions, restype_name_to_atomc_names,get_chi_atom_indices, chi_pi_periodic, atom_order,restype_1_to_index
from CryoAtom2.utils.affine_utils import affine_from_3_points, get_affine_rot, get_affine, affine_mul_vecs, invert_affine, \
    init_random_affine_from_translation
from CryoAtom2.utils.torch_utlis import batched_gather
from CryoAtom2.utils.residue_constants import chi_angles_mask as chi_mask
def load_cas_c1s_from_structure(stu_fn, all_structs=False, quiet=True):
    ...
    for model in structure:
        ...
        for i, a in enumerate(model.get_atoms()):
            aname = a.get_name()
            if aname == "CA" or aname == 'P' or aname == "C1'" or aname == 'N' or aname == 'O' or aname == 'N1' or aname == 'N9':
    ...
    return {k:np.array(v).reshape(-1,3) for k,v in coords.items()}
def load_affines_cas_from_structure(stu_fn, all_structs=False, quiet=True):
    ...
    return rigidgroups_gt_frames.numpy(),ncac_coords,only_ca_coords
                    ...
                    if residue.has_id('N') and residue.has_id('CA') and residue.has_id('C'):
                        ...
                        if aa_name == 'Z':
                            ...
                            for atom_name in rigid_group_atom_positions[resname]:
                                ...
                                atom14idx = restype_name_to_atomc_names[resname].index(atom_name)
                                try:
