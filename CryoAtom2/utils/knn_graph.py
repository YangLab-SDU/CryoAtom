"""
This file removes the need for torch_cluster, but it forces this operation to CPU.
For inference, this is ok. For training, we still need torch_cluster for the speed.

The code is copied from: https://pytorch-geometric.readthedocs.io/en/1.3.1/_modules/torch_cluster/knn.html
"""
import torch
...
def knn(x, y, k, batch_x=None, batch_y=None):
    ...
    dist, col = tree.query(
        y.cpu().detach().numpy(), k=k, distance_upper_bound=x.size(1))
